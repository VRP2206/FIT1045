...
int main(){
    ...
    d_arr<int> *arr = new_dynamic_array<int>(10);
    
    for (int i = 0; i < 15; i++){
        add(arr,i);
    }

    write_line("Size: " + std::to_string(arr->size));
    write_line("capacity: " + std::to_string(arr->capacity));

    for(unsigned int i = 0; i < arr->size; i++){
        int val = get(arr,i);
        write_line(std::to_string(i)+ " = " + std::to_string(val));
        set(arr,i, val*2);
    }

    try{
        int val = get(arr,99);
        write_line("ERROR! array[99] = " + std::to_string(val));
    }
