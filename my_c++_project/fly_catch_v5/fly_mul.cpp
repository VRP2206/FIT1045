...
void update_game(game_data &game, long current_time, int &countwin, int &countgame)
{
    for (int i = 0; i < 5; i++)  
    {
        update_fly(game.flies[i], current_time, countgame);  
 ...
 * @param countwin of data type int passed by value; counts the numeber od game won
 * @param countgame of data type int passed by reference; counts the number of games
*/
void draw_game(game_data game, int countwin, int &countgame)
{
    ...
    draw_text("You eaten " + std::to_string(countwin) + " out of " + std::to_string(countgame) + " flies!", color_black(), "main", 60, 10, 10);
...
int main()
{
    ...
    open_window("Fly Catch", GAME_SCREEN_WIDTH, SCREEN_HEIGHT);
